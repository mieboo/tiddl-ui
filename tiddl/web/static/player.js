...
function singleTrackRow(track,index) { return `<button class="queue-item ${index===state.current?"active":""}${String(track.id)===String(state.selectedTrackId)?" selected":""}" type="button" data-play="${index}" data-track-id="${esc(String(track.id))}">${imgSrc(track.cover)?`<img src="${esc(imgSrc(track.cover))}" alt="">`:`<span class="queue-cover-placeholder"><i data-lucide="music"></i></span>`}<span class="queue-copy">${tScroll(track.title,"strong")}${artistLink(track)}</span><span class="queue-duration">${formatTime(track.duration)}</span><span class="queue-remove queue-fav icon-button${isTrackFavorited(track)?" fav-on":""}" data-fav-track="${index}" title="Favorite"><i data-lucide="heart"></i></span><span class="queue-remove icon-button" data-remove="${index}" title="Remove"><i data-lucide="x"></i></span></button>`; }
...
function favoriteRow(entry,index) {
  ...
  return `<button class="queue-item fav-item${pending?" pending":""}" type="button" data-fav-play="${index}">${cover}<span class="queue-copy">${tScroll(entry.title,"strong")}${artistLink(entry)}</span><span class="queue-duration">${formatTime(entry.duration)}</span>${heart}${add}</button>`;
}
function innerFavTrack(track,i,entry) {
  ...
  return `<button class="playlist-track-row fav-track-row${excluded?" excluded":""}" type="button" data-fav-inner-row="${i}"><span class="track-number">${track.track_number||i+1}</span><span class="track-name">${tScroll(track.title,"strong")}${artistLink(track)}</span><span class="track-duration">${formatTime(track.duration)}</span><span class="queue-remove queue-fav icon-button${covered?" fav-on":""}" data-inner-fav title="${excluded?t("favorite"):t("unfavorite")}"><i data-lucide="heart"></i></span>${addIcon(trackInQueue(track.id)?"full":"none",`data-inner-add="${i}"`)}</button>`;
}
...
function showPanelTab(tab) { state.panelTab=tab; document.querySelectorAll("[data-panel-tab]").forEach(button=>button.classList.toggle("active",button.dataset.panelTab===tab)); $("#lyrics").hidden=tab!=="lyrics"; $("#artistView").hidden=tab!=="artist"; $("#infoView").hidden=tab!=="info"; if(tab==="info")renderInfo(); if(tab==="artist")renderArtistView(); }
function renderInfo() {
  const view=$("#infoView");
...
function artistRow(entry,index) {
  ...
  const tracks=entry._tracks?entry._tracks.map((track,i)=>`<button class="playlist-track-row" type="button" data-artist-track="${i}" data-album-index="${index}"><span class="track-number">${track.track_number||i+1}</span><span class="track-name">${tScroll(track.title,"strong")}${artistLink(track)}</span><span class="track-duration">${formatTime(track.duration)}</span>${addIcon(trackInQueue(track.id)?"full":"none",`data-artist-inner-add="${i}" data-album-index="${index}"`)}</button>`).join(""):`<div class="fav-album-loading">${t("loadingTracks")}</div>`;
...
function artistTrackRow(track,index) {
  ...
  return `<button class="queue-item" type="button" data-artist-track-play="${index}">${cover}<span class="queue-copy">${tScroll(track.title,"strong")}${artistLink(track)}${track.album?tScroll(track.album,"span"):""}</span><span class="queue-duration">${formatTime(track.duration)}</span>${heart}${add}</button>`;
}
...
function actualSpecText() { const info=state.currentInfo; if(!info)return ""; const parts=[]; if(info.bit_depth)parts.push(`${info.bit_depth} bit`); if(info.sample_rate)parts.push(`${(info.sample_rate/1000).toFixed(1)} kHz`); if(info.bitrate)parts.push(`${info.bitrate} kbps`); return parts.join(" · "); }
function renderQualityControl() { const actual=state.actualQuality; const spec=actualSpecText(); const selSpec=qualitySpecs[state.quality]||{label:String(state.quality||""),tone:"default"}; const label=spec?`${selSpec.label} · ${spec}`:selSpec.label; const selected=actual?.audioMode==="DOLBY_ATMOS"?{label:`Dolby Atmos${actual.transcoded?" (stereo)":""} · ${actual.codec||"immersive"}`,tone:"crimson"}:{label,tone:selSpec.tone}; const choices=state.availableQualities.filter(value=>qualitySpecs[value]); const locked=state.defaultHighest||state.defaultLowest; const disabled=state.current<0&&!locked; const lockTitle=locked?t(state.defaultLowest?"defaultLowestToast":"defaultHighestToast"):""; const menuOpen=!($("#qualityMenu")||{hidden:true}).hidden; $("#qualityControl").innerHTML=`<div class="spec-control"><button id="qualityButton" class="spec-tag spec-tone-${selected.tone}${locked?" locked":""}" type="button" ${disabled?"disabled":""} title="${esc(lockTitle)}"><span>${esc(selected.label)}</span><i data-lucide="${locked?"lock":"chevron-down"}"></i></button><div id="qualityMenu" class="spec-menu" ${menuOpen?"":"hidden"}>${choices.map(value=>`<button type="button" class="${value===state.quality?"selected":""}" data-quality-choice="${value}"><span>${esc(qualitySpecs[value].label)}</span>${value===state.quality?`<i data-lucide="check"></i>`:""}</button>`).join("")}</div></div>`; icon(); }
...
function updateQualityOptions(track) { state.availableQualities=[...(track.qualities||[])]; state.actualQuality=null; if(state.defaultLowest)state.quality=state.availableQualities[0]||"LOW";else if(state.defaultHighest)state.quality=state.availableQualities.at(-1);else if(!track.qualities.includes(state.quality))state.quality=chooseQuality(track); if(!state.selectedQuality||!state.availableQualities.includes(state.selectedQuality))state.selectedQuality=state.quality; renderQualityControl(); }
function showActualQuality(data) {
  // 用 resolve 返回的真实支持音质刷新菜单(搜索/预览数据可能只带 LOW/HIGH)
  const real = data && data.track && Array.isArray(data.track.qualities) ? data.track.qualities.filter((q) => qualitySpecs[q]) : null;
  if (real && real.length) {
    state.availableQualities = real;
    if (state.defaultLowest) state.quality = real[0];
    else if (state.defaultHighest) state.quality = real.at(-1);
    else if (!real.includes(state.quality)) state.quality = chooseQuality({ qualities: real });
  }
  if(data.transcoded){state.actualQuality={audioMode:"DOLBY_ATMOS",codec:data.codec,transcoded:true};} else if(data.audio_mode==="DOLBY_ATMOS"){state.actualQuality={audioMode:data.audio_mode,codec:data.codec};} else {state.actualQuality=null;state.quality=data.audio_mode==="DOLBY_ATMOS"?joinQuality(data.quality,true):data.quality;} renderQualityControl(); }
...
function loadArtistAlbumBox(head) {
  ...
  loadFavoriteAlbumTracks(entry).then(tracks=>{
    if(tracks){
      box.innerHTML=tracks.map((track,i)=>`<button class="playlist-track-row" type="button" data-artist-track="${i}" data-album-index="${Number(details.dataset.artistAlbum)}"><span class="track-number">${track.track_number||i+1}</span><span class="track-name">${tScroll(track.title,"strong")}${artistLink(track)}</span><span class="track-duration">${formatTime(track.duration)}</span>${addIcon(trackInQueue(track.id)?"full":"none",`data-artist-inner-add="${i}" data-album-index="${Number(details.dataset.artistAlbum)}"`)}</button>`).join("");
