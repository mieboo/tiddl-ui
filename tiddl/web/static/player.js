...
function singleTrackRow(track,index) { return `<button class="queue-item ${index===state.current?"active":""}${String(track.id)===String(state.selectedTrackId)?" selected":""}" type="button" data-play="${index}" data-track-id="${esc(String(track.id))}">${imgSrc(track.cover)?`<img src="${esc(imgSrc(track.cover))}" alt="">`:`<span class="queue-cover-placeholder"><i data-lucide="music"></i></span>`}<span class="queue-copy">${explicitTitle(track.title,track.explicit,"strong")}${artistLink(track)}</span><span class="queue-duration">${formatTime(track.duration)}</span><span class="queue-remove queue-fav icon-button${isTrackFavorited(track)?" fav-on":""}" data-fav-track="${index}" title="Favorite"><i data-lucide="heart"></i></span><span class="queue-remove icon-button" data-remove="${index}" title="Remove"><i data-lucide="x"></i></span></button>`; }
...
function renderResults(results) { const panel=$("#playerSearchResults"); panel.innerHTML=results.length?results.map(result=>{const[rk,rid]=result.resource.split("/");const artists=artistsOf(result);const artistsAttr=artists.length?` data-artists="${esc(JSON.stringify(artists))}"`:"";return `<button class="player-result" type="button" data-resource="${esc(result.resource)}"${artistsAttr}>${imgSrc(result.cover)?`<img src="${esc(imgSrc(result.cover))}" alt="">`:`<span></span>`}<span class="player-result-copy">${explicitTitle(result.title,result.explicit,"strong")}${artistLink(result)||tScroll(result.subtitle,"span")}</span><span class="result-fav icon-button${isFavorite(rk,rid)?" fav-on":""}" data-result-fav="${rk}:${rid}" title="Favorite"><i data-lucide="heart"></i></span><span class="result-type">${t(result.type)}</span></button>`;}).join(""):`<div class="player-empty">${t("noResults")}</div>`; panel.hidden=false; markOverflowTitles(); }
...
function favoriteRow(entry,index) {
  ...
  return `<button class="queue-item fav-item${pending?" pending":""}" type="button" data-fav-play="${index}">${cover}<span class="queue-copy">${explicitTitle(entry.title,entry.explicit,"strong")}${artistLink(entry)}</span><span class="queue-duration">${formatTime(entry.duration)}</span>${heart}${add}</button>`;
}
function innerFavTrack(track,i,entry) {
  ...
  return `<button class="playlist-track-row fav-track-row${excluded?" excluded":""}" type="button" data-fav-inner-row="${i}"><span class="track-number">${track.track_number||i+1}</span><span class="track-name">${explicitTitle(track.title,track.explicit,"strong")}${artistLink(track)}</span><span class="track-duration">${formatTime(track.duration)}</span><span class="queue-remove queue-fav icon-button${covered?" fav-on":""}" data-inner-fav title="${excluded?t("favorite"):t("unfavorite")}"><i data-lucide="heart"></i></span>${addIcon(trackInQueue(track.id)?"full":"none",`data-inner-add="${i}"`)}</button>`;
}
...
function artistRow(entry,index) {
  ...
  const tracks=entry._tracks?entry._tracks.map((track,i)=>`<button class="playlist-track-row" type="button" data-artist-track="${i}" data-album-index="${index}"><span class="track-number">${track.track_number||i+1}</span><span class="track-name">${explicitTitle(track.title,track.explicit,"strong")}${artistLink(track)}</span><span class="track-duration">${formatTime(track.duration)}</span>${addIcon(trackInQueue(track.id)?"full":"none",`data-artist-inner-add="${i}" data-album-index="${index}"`)}</button>`).join(""):`<div class="fav-album-loading">${t("loadingTracks")}</div>`;
...
function artistTrackRow(track,index) {
  ...
  return `<button class="queue-item" type="button" data-artist-track-play="${index}">${cover}<span class="queue-copy">${explicitTitle(track.title,track.explicit,"strong")}${artistLink(track)}${track.album?tScroll(track.album,"span"):""}</span><span class="queue-duration">${formatTime(track.duration)}</span>${heart}${add}</button>`;
}
...
function qualityMenuLabel(q) { const base=qualitySpecs[q]?qualitySpecs[q].label:String(q||""); const bw=state.formatBandwidths; if(!bw)return base; const fmt={LOSSLESS:"FLAC",HI_RES_LOSSLESS:"FLAC_HIRES",LOSSLESS_ATMOS:"FLAC",HI_RES_LOSSLESS_ATMOS:"FLAC_HIRES",HIGH:"AACLC",LOW:"HEAACV1"}[q]; if(!fmt||!bw[fmt])return base; return `${base} · ${bw[fmt]} kbps`; }
...
function loadArtistAlbumBox(head) {
  ...
  loadFavoriteAlbumTracks(entry).then(tracks=>{
    if(tracks){
      box.innerHTML=tracks.map((track,i)=>`<button class="playlist-track-row" type="button" data-artist-track="${i}" data-album-index="${Number(details.dataset.artistAlbum)}"><span class="track-number">${track.track_number||i+1}</span><span class="track-name">${explicitTitle(track.title,track.explicit,"strong")}${artistLink(track)}</span><span class="track-duration">${formatTime(track.duration)}</span>${addIcon(trackInQueue(track.id)?"full":"none",`data-artist-inner-add="${i}" data-album-index="${Number(details.dataset.artistAlbum)}"`)}</button>`).join("");
